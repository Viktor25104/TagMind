...
1) `cp env/local.compose.env.example env/local.compose.env` and fill secrets if needed (stubs do not require them).
2) `make compose-up`
3) Health: `make check` or curl `http://localhost:8081/healthz` .. `/8084/healthz`.
4) Smoke: `./scripts/dev-smoke.sh` (hits all services with deterministic requests).
